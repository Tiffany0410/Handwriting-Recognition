...
int main(int argc, char *argv[]) {
    ...
    if (strncmp(dist_metric, "euclidean", strlen(dist_metric)) == 0){
        ...
        fprintf(stderr, "No matching distance function");
    }
    ...
    // create copies
    int num_items = testing->num_items;
    int num_pro = num_procs;

    // TODO
    int fds1[num_procs][2]; // For index
    int fds2[num_procs][2]; // For result

    for (int i = 0; i < num_procs; i++){
        // Calculate N
        int N = ceil(num_items/num_pro);
		...
		if (result < 0) {
            ...
            // Child only read from pipe, so close writing end for pipe1
            if (close(fds1[i][1]) == -1){
                perror("close");
                exit(1);
            }

            // Close reading end for pipe 2
            if (close(fds2[i][0]) == -1){
                perror("close");
                exit(1);
            }

            // Before we forked, parent had open the writing ends to
            ...
            // Call child handler
            child_handler(training, testing, K, fptr, fds1[i][0], fds2[i][1]);

            // Close the reading pipe1 and writing pipe2
            if (close(fds1[i][0]) == -1){
                perror("close");
                exit(1);
            }
            ...
            // In the parent, close reading end for pipe1
            if (close(fds1[i][0]) == -1){
                perror("close");
                exit(1);
            }

            // In the parent, close writing end for pipe2
            if (close(fds2[i][1]) == -1){
                perror("close");
                exit(1);
            }

            // Write start_idx to pipe
			int start_idx = testing->num_items - num_items;
            if (write(fds1[i][1], &start_idx, sizeof(int)) != sizeof(int)){
                perror("write from parent to pipe");
                exit(1);
            }

            // Write N to pipe
            if (write(fds1[i][1], &N, sizeof(int)) != sizeof(int)){
                perror("write from parent to pipe");
                exit(1);
            }

            // Close writing end for pipe1 after finish
            if (close(fds1[i][1]) == -1){
                perror("close");
                exit(1);
            }
    ...
    for (int i = 0; i < num_procs; i++){
        if (wait(&status) == -1){   // No child processes
            perror("wait");
            exit(1);
        }
        if (WIFEXITED(status)){
            int count;
            if (read(fds2[i][0], &count, sizeof(int)) != sizeof(int)){
                perror("read");
                exit(1);
            }
            if (close(fds2[i][0]) == -1){
