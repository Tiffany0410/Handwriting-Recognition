...
void child_handler(Dataset *training, Dataset *testing, int K, 
    ...
    int stop;
    if (testing->num_items < (start_idx + N-1)){
        stop = testing->num_items;
    }
    else{
        stop = (start_idx + N);
    }
    for (int i = start_idx; i < stop; i++){
        int result = knn_predict(training, &testing->images[i], K, fptr);
        if (result == testing->labels[i]){
            correct++;
        }
    }

    write(p_out, &correct, sizeof(int));
...
double distance_cosine(Image *a, Image *b){
    ...
    double sum_y = 0;
    for (int i = 0; i < a->sx * a->sy; i++) {
        sum_xy += a->data[i] * b->data[i];
        sum_x += a->data[i] * a->data[i];
        sum_y += b->data[i] * b->data[i];
    }
    double d = sum_xy / (sqrt(sum_x) * sqrt(sum_y));
    return (acos(d) * 2)/M_PI;
}